...
export namespace Sdk {
	export function is(value: Sdk | any): value is Sdk {
		return (
			...
			value.encryptedData.length <= 64000 &&
			value.ephemeralPublicKey != undefined &&
			typeof value.timeout == "string" &&
			value.timeout.length == 2 &&
			/^(([1-9]\d)|(0[5-9]))$/.test(value.timeout) &&
			typeof value.reference == "string" &&
			value.reference.length <= 32 &&
			typeof value.transaction == "string" &&
			(value.interface == undefined || Range03.is(value.interface)) &&
			(value.ui == undefined ||
				(Array.isArray(value.ui) && value.ui && value.ui.every((type: any) => Range05.is(type))))
		)
	}
}
