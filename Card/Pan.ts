...
export namespace Pan {
	...
	export function scheme(pan: Pan): Scheme {
		...
		for (const key in schemes)
			if (
				Object.prototype.hasOwnProperty.call(schemes, key) &&
				schemes[key].identification.test(pan) &&
				Scheme.is(key)
			) {
				result = key
				break
...
const schemes: { [scheme: string]: { verification: RegExp; identification: RegExp; length: number; icon: string } } = {
	...
	diners: {
		verification: /^3(?:0[0-5]|[68][0-9])[0-9]{11}$/,
		identification: /^3(?:0[0-5]|[68][0-9])/,
		length: 14,
		icon: "diners",
	},
	discover: {
		verification: /^6(?:011|5[0-9]{2})[0-9]{12}$/,
		identification: /^6(?:011|5[0-9]{2})/,
		length: 19,
		icon: "generic",
	},
	electron: {
		verification: /^(4026|417500|4405|4508|4844|4913|4917)\d+$/,
	...
	jcb: {
		verification: /^(?:2131|1800|35\d{3})\d{11}$/,
		identification: /^(?:2131|1800|35\d{3})/,
		length: 19,
		icon: "generic",
	},
	maestro: {
		verification: /^(5018|5020|5038|5612|5893|6304|6759|6761|6762|6763|0604|6390)\d+$/,
