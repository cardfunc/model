		...
		return (
			...
			(value.country == undefined ||
				(typeof value.country == "string" &&
					(value.country.length == 2 || value.country.length == 3) &&
					(isoly.CountryCode.Numeric.is(value.country.toLocaleUpperCase()) ||
						isoly.CountryCode.Numeric.from(value.country.toLocaleUpperCase())))) &&
	...
	function compareOneProperty(first: string | undefined, second: string | undefined): boolean {
		return (
			...
			first
				?.trim()
				.toLocaleUpperCase()
				.localeCompare(second?.trim().toLocaleUpperCase() ?? "") == 0
		)
	}
}
